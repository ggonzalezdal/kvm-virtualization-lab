...
A hands-on learning repository focused on \*\*\*\*KVM, QEMU, libvirt,

Alpine Linux, Linux system administration, networking, virtualization,

containers, automation, and LPIC-1 preparation\*\*\*\*.

The objective of this project is not simply to learn commands, but to

build, document, automate, and understand a complete virtual

infrastructure using industry-relevant practices.
...
Automated provisioning is performed independently through the Linux Mint

KVM host and the libvirt `default` network:
...
| \- Additional qcow2 data disk created and attached persistently        |
| \- Guest block-device identification and `vda` / `vdb` enumeration     |
|   behavior                                                             |
...
|----------------------------------------------------------------------------|
| \## Project Status — Complete \*\*\*\*Status:\*\*\*\* ✅ Complete          |
| The KVM Virtualization Lab concludes with \*\*Phase 10 — VM Provisioning & |
| Automation\*\*.                                                            |
| The project progressed from basic KVM/libvirt administration to a          |
| complete multi-VM virtual infrastructure including networking, routing,    |
| DHCP/DNS, firewalling, Linux services, service publishing, storage         |
| management, containers, and automated VM provisioning.                     |
| Further work involving configuration management, fleet administration,     |
| monitoring, backups, centralized logging, certificates/TLS, and related    |
| operational practices will continue in a separate \*\*Linux Infrastructure |
| Administration Lab\*\*.                                                    |
...
This keeps the laboratory recoverable and records known-good checkpoints

without creating unnecessary snapshots.
...
| Project status:                                                    |
| \- ✅ All planned KVM Virtualization Lab technical phases complete |
| \- ✅ Final technical checkpoint reached with Phase 10             |
| \- ➡️ Fleet administration and operations continue in the separate |
|   \*\*\*\*Linux Infrastructure Administration Lab\*\*\*\*          |
| The environment is:                                                |
